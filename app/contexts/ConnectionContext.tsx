...
function describeWebSocketErrorEvent(event: unknown): Record<string, unknown> {
  ...
  return {
    ...
    nativeMessage:
      typeof candidate.nativeEvent?.message === "string"
        ? candidate.nativeEvent.message
        : null,
    nativeCode:
      typeof candidate.nativeEvent?.code === "number"
        ? candidate.nativeEvent.code
        : null,
    nativeReason:
      typeof candidate.nativeEvent?.reason === "string"
        ? candidate.nativeEvent.reason
        : null,
    nativeType:
      typeof candidate.nativeEvent?.type === "string"
        ? candidate.nativeEvent.type
        : null,
...
function normalizeGateway(input: string): string {
  ...
  if (lower.startsWith("ws://") || lower.startsWith("http://")) {
    throw new Error(
      "Insecure gateway protocol is not allowed; use wss:// or https://",
    );
  }
...
function parseConnectPayload(value: string): {
  code: string;
  password?: string;
} {
...
export function ConnectionProvider({
  ...
  const [interactionBlockReason, setInteractionBlockReason] = useState<
  ...
  const sendControlRef = useRef<
    ...
    | null
  >(null);
  const reconnectWithPasswordRef = useRef<
    (() => Promise<{ ok: boolean; terminal: boolean; message?: string }>) | null
  >(null);
  const runReconnectLoopRef = useRef<
    | ((
        source: "app_active" | "network_restored" | "transport_closed",
      ) => Promise<void>)
    | null
  >(null);
  ...
  const setReconnectUiState = useCallback(
    (reason: "offline" | "reconnecting" | null) => {
  ...
  const persistPairedSessions = useCallback(
    async (sessions: PairedSession[]): Promise<void> => {
      ...
      try {
        ...
        logger.warn(
          "connection",
          "failed to persist paired sessions to secure store",
          {
            key: PAIRED_SESSIONS_STORAGE_KEY,
            payloadBytes: raw.length,
            error:
  ...
  const removePairedSession = useCallback(
    async (secret: string): Promise<void> => {
      const existing = await getPairedSessions();
      await persistPairedSessions(
        existing.filter((entry) => entry.sessionPassword !== secret),
      );
    },
  ...
  const revokePairedSession = useCallback(
    async (secret: string): Promise<void> => {
      const response = await fetch(
        new URL("/v2/revoke", MANAGER_URL).toString(),
        {
          method: "POST",
  ...
  const savePairedSession = useCallback(
    async (
      ...
      const previous = existing.find(
        (entry) => entry.sessionPassword === session.sessionPassword,
      );
  ...
  const connectToGatewayV2 = useCallback(
    async (
      gateway: string,
      options?: {
          ...
          onSystemMessage: async (message) => {
            ...
            if (message.type === "peer_connected") {
              ...
              setSessionState((current) =>
                current === "cli_offline_grace" ? "pending" : current,
              );
              setError(null);
              logger.info(
                "connection",
                "evaluating proxy configuration on peer connect",
                {
              ...
              if (sessionCodeRef.current && sendControlRef.current) {
                ...
                logger.warn(
                  "connection",
                  "proxy not configured on peer connect",
                  {
                    reason: !sessionCodeRef.current
                      ? "missing_session_code"
                      : "missing_send_control",
                    hasSessionCode: Boolean(sessionCodeRef.current),
                    hasSessionPassword: Boolean(sessionPasswordRef.current),
                    hasSendControl: Boolean(sendControlRef.current),
                  },
            ...
            if (message.type === "peer_disconnected") {
              logger.warn("connection", "peer disconnected", {
                peer: message.peer ?? null,
              });
              stopAllServers();
              ...
              setError(
                message.peer === "cli"
                  ? "CLI is having some issues. Trying to reconnect..."
                  : "Peer disconnected",
              );
              return;
            }
            ...
            if (message.type === "close_connection") {
              ...
              stopAllServers();
              setSessionState(
                (message.reason || "").toLowerCase().includes("expired")
                  ? "expired"
                  : "ended",
              );
              setStatus("error");
              ...
              if (
                (message.reason || "").includes("ended") ||
                (message.reason || "").includes("expired")
              ) {
            ...
            if (
              ...
              if (Array.isArray(ports)) {
                ...
                if (AppState.currentState === "active") {
                  logger.info(
                    "connection",
                    "starting localhost proxy servers from discovered ports",
                    {
                      ports,
                      appState: AppState.currentState,
                    },
                  ...
                  logger.info(
                    "connection",
                    "deferring localhost proxy server start until app active",
                    {
                      ports,
                      appState: AppState.currentState,
                    },
      ...
      if (sessionPasswordRef.current) {
        try {
          await savePairedSession(
            ...
            {
              hostname: String(
                (response.payload as Record<string, unknown>).hostname ?? "",
              ),
              rootDir: String(
                (response.payload as Record<string, unknown>).rootDir ?? "",
              ),
            },
          );
        } catch (error) {
  ...
  const assembleWithCode = useCallback(
    async (code: string): Promise<AssembleResult> => {
      ...
      try {
        logger.info(
          "connection",
          "probing manager health before assemble websocket",
          {
            code,
            healthUrl,
          },
        ...
        logger.warn(
          "connection",
          "manager health probe failed before assemble websocket",
          {
            code,
            healthUrl,
            error: error instanceof Error ? error.message : String(error),
  ...
  const getAssignedProxyUrl = useCallback(
  ...
  const claimReattach = useCallback(
    async (password: string): Promise<ReattachClaimResult> => {
      const response = await fetch(
        new URL("/v2/reattach/claim", MANAGER_URL).toString(),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const reconnectWithPassword = useCallback(async (): Promise<{
    ok: boolean;
    terminal: boolean;
    message?: string;
  }> => {
    ...
    if (!sessionPasswordRef.current) {
      logger.info(
        "connection",
        "reconnect skipped: no session password in memory",
      );
      return {
        ok: false,
        terminal: true,
        message: "No in-memory session to reconnect",
      };
    }
  ...
  const runReconnectLoop = useCallback(
    async (source: "app_active" | "network_restored" | "transport_closed") => {
      ...
      if (!sessionPasswordRef.current) {
        logger.info(
          "connection",
          "reconnect loop skipped: no in-memory session",
          { source },
        );
        return;
      }
      ...
      try {
        ...
        while (!manualDisconnectRef.current && sessionPasswordRef.current) {
          if (!networkReachableRef.current) {
            logger.info("connection", "reconnect loop paused while offline", {
              source,
            });
          ...
          const baseDelay = Math.min(
            500 * 2 ** Math.min(reconnectAttemptRef.current, 5),
            5_000,
          );
  ...
  const handleConnectivityLost = useCallback(() => {
    ...
    logger.warn(
      "connection",
      "manager reachability lost; entering offline state",
    );
  ...
  const connect = useCallback(
    async (payload: string) => {
      logger.info("connection", "connect requested", {
        payloadPreview: payload.trim().slice(0, 32),
      });
  ...
  const resumeSession = useCallback(
    async (stored: StoredSession) => {
      ...
      if (!stored?.sessionPassword) {
        logger.error(
          "connection",
          "resume rejected due to invalid stored session",
        );
        setSessionState("ended");
      ...
      try {
        ...
        gatewaysRef.current = [reattach.proxyUrl];
        reattachGenerationRef.current = reattach.generation;
        logger.info("connection", "resolved resume gateways", {
          gateways: gatewaysRef.current,
        });
      } catch (err) {
        const msg = err instanceof Error ? err.message : "Resume failed";
        logger.error("connection", "manager proxy lookup failed", {
          error: msg,
        });
        setSessionState(
          err instanceof ProxyLookupError && err.status === 404
            ? "expired"
            : "ended",
        );
        setStatus("error");
      ...
      if (nextState === "active") {
        ...
        if (
          ...
          if (sessionPasswordRef.current) {
            logger.info(
              "connection",
              "app returned active while disconnected; retrying reconnect from memory",
            );
            void runReconnectLoop("app_active");
          } else {
            logger.info(
              "connection",
              "app returned active without in-memory session; skipping automatic reconnect",
            );
          }
        }
        return;
